...
module.exports.qaRegistrationValidation = [
  /*------------validation for Name--------------*/
  ...
  */
  ...
  body("areaOfExpertise")
    .notEmpty()
    .withMessage("Please enter area of Expertise")
    .bail()
    .isArray({min: 1})
    .withMessage("Area of Expertise must me in array")
    .bail()
    .custom(async (value, { req }) => {
  ...
  body("qualityAssuranceProcedures")
    .notEmpty()
    .withMessage("Please enter quality Assurance Procedures")
    .bail()
    .isArray({min: 1})
    .withMessage("quality Assurance procedures must me in an array")
    .bail()
    .custom(async (value, { req }) => {
      for (let index in value){
        value[index] = value[index].trim();
  ...
  try {
      ...
      jwt.verify(jwt_token, process.env.JWT_SECRET_KEY, async (err, data) => {
          if (err) {
              ...
              const qaFromCC = await chaincode.invokeChaincode("fetchRecordWithEmail", data.data.email, true);
              ...
              if(qaFromCC.data[0].role != 'qualityAnalyst'){
                return res.status(statusCode.Bad_request).json({ Messages: messages.statusError, ResponseStatus: responseStatus.failure })
              }

              if(qaFromCC.profileStatus != "active"){
                return res.status(statusCode.Bad_request).json({ Messages: messages.statusError, ResponseStatus: responseStatus.failure })
              }

              req.email = data.email;
              req.profileStatus = qaFromCC.data[0].profileStatus;
              next();
          }
      })
